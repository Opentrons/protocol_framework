...
class FlexStacker:
    ...
    @classmethod
    def create_from_sn(
        cls, sn: str, baudrate: int = 115200, timeout: float = 1.0
    ) -> "FlexStacker":
        """Flex Stacker Driver"""
        port_name = None
        ...
        if not port_name:
            raise ValueError(
                f"Could not find connected stacker with serial number {sn}"
            )

        return cls.create(port=port_name, baudrate=baudrate, timeout=timeout)
    ...
    def get_device_info(self) -> str:
        ...
        response = self.send_command(command=c, retries=DEFAULT_COMMAND_RETRIES).strip(
            "OK"
        )
        return response
    ...
    def set_device_serial_number(self, serial_number) -> None:
        ...
        c = (
            CommandBuilder(terminator=FS_COMMAND_TERMINATOR)
            .add_gcode(gcode=GCODE.SET_SERIAL_NUM)
            .add_element(serial_number)
        )
        response = self.send_command(command=c, retries=DEFAULT_COMMAND_RETRIES).strip(
            "OK"
        )
    ...
    def enable_motor(self, axis: AXIS):
        ...
        c = (
            CommandBuilder(terminator=FS_COMMAND_TERMINATOR)
            .add_gcode(gcode=GCODE.ENABLE_MOTOR)
    ...
    def disable_motor(self, axis: AXIS):
        ...
        c = (
            CommandBuilder(terminator=FS_COMMAND_TERMINATOR)
            .add_gcode(gcode=GCODE.DISABLE_MOTOR)
            .add_element(axis.upper())
        )
        response = self.send_command(command=c, retries=DEFAULT_COMMAND_RETRIES).strip(
            "OK"
    ...
    def get_current_motion_params(self, axis) -> str:
        ...
        c = (
            CommandBuilder(terminator=FS_COMMAND_TERMINATOR)
            .add_gcode(gcode=GCODE.CURRENT_MOTION_PARMS)
            .add_element(axis)
        )
        response = self.send_command(command=c, retries=DEFAULT_COMMAND_RETRIES).strip(
            "CMD: rrr"
    ...
    def move(
        ...
        c = (
            CommandBuilder(terminator=FS_COMMAND_TERMINATOR)
            .add_gcode(gcode=GCODE.MOVE_DIST)
            .add_element(axis.upper() + f"{direction}" + f"{distance}")
            .add_element(f"V{velocity}")
            .add_element(f"A{acceleration}")  # )
            .add_element(f"D{msd}")
        )
    ...
    def microstepping_move(
        ...
        c = (
            CommandBuilder(terminator=FS_COMMAND_TERMINATOR)
            .add_element(axis.upper())
            .add_element(f"{direction}")
            .add_gcode(gcode=GCODE.MOVE_IGNORE_LIMIT)
        )
    ...
    def set_microstep(self, axis: AXIS, microstepping: int):
        c = (
            ...
            .add_element(f"{axis.upper()}{microstepping}")
        )
        self.send_command(command=c, retries=DEFAULT_COMMAND_RETRIES)
    ...
    def home(
        ...
        c = (
            CommandBuilder(terminator=FS_COMMAND_TERMINATOR)
            .add_gcode(gcode=GCODE.MOVE_LS)
            .add_element(axis.upper() + direction)
            .add_element(f"V{velocity}")
            .add_element(f"A{acceleration}")
        )
        self.send_command(command=c, retries=DEFAULT_COMMAND_RETRIES)
    ...
    def set_ihold_current(self, current: float, axis: AXIS) -> str:
        ...
        c = (
            CommandBuilder(terminator=FS_COMMAND_TERMINATOR)
            .add_gcode(gcode=GCODE.SET_IHOLD_CURRENT)
    ...
    def set_run_current(self, current: float, axis: AXIS) -> str:
        ...
        c = (
            CommandBuilder(terminator=FS_COMMAND_TERMINATOR)
            .add_gcode(gcode=GCODE.SET_PEAK_CURRENT)
            .add_element(axis + f"{current}")
        )
        self.send_command(command=c, retries=DEFAULT_COMMAND_RETRIES)
        time.sleep(0.1)
    ...
    def load_labware(self, labware_height: float):
        ...
        self.move(
            ...
            DIR.NEGATIVE,
            self.move_speed_x,
            self.move_acceleration_x,
        )
        self.home(AXIS.X, DIR.NEGATIVE_HOME, HOME_SPEED, HOME_ACCELERATION)
        self.move(
            AXIS.Z,
            TOTAL_TRAVEL_Z - (labware_height / 2) - 10,
            DIR.POSITIVE,
            self.move_speed_up_z / 4,
            self.move_acceleration_z,
        )
        # #------------------- transfer -----------------------------
        self.open_latch()
        self.move(
    ...
    def unload_labware(self, labware_height: float):
        ...
        self.move(
            AXIS.Z,
            TOTAL_TRAVEL_Z - (labware_height / 2 + axis_swap_approach_mm),
            DIR.NEGATIVE,
            self.move_speed_down_z,
            self.move_acceleration_z,
        )
    ...
    def write_to_motor_drive(self, register, data):
        ...
        c = (
            CommandBuilder(terminator=FS_COMMAND_TERMINATOR)
            .add_gcode(gcode=GCODE.WRITE_TO_REGISTER)
            .add_element(axis.upper())
            .add_element(register)
            .add_element(data)
        )
    ...
    def enable_SG(self, axis: AXIS, sg_value: int, enable: bool):
        ...
        *: optional params, default/previously set values will be used if
        ...
        c = (
            CommandBuilder(terminator=FS_COMMAND_TERMINATOR)
            .add_gcode(gcode=GCODE.STALLGUARD)
            .add_element(f"{axis.upper()}{int(enable)}")
            .add_element(f"T{sg_value}")
        )

...
class TipPickUpParams(BaseModel):
    prepDistance: float = Field(default=8.25, description="The distance to move down to fit the tips on.")
    pressDistance: float = Field(default=3.5, description="The distance to press on tips.")
    ejectorPushmm: float = Field(default=7.0, description="The distance to back off to ensure that the tip presence sensors are not triggered.")
...
class EvotipSealPipetteParams(PipetteIdMixin):
    ...
    tipPickUpParams: Optional[TipPickUpParams] = Field(default=None, description="Specific parameters for ")
...
class EvotipSealPipetteImplementation(
    ...
    async def relative_pickup_tip(self, pipette_id: str, tip_geometry: TipGeometry, tip_pick_up_params: TipPickUpParams, mount: MountType, press_fit: bool) -> None:
        ...
        if press_fit:
            await self._gantry_mover.move_axes(axis_map={mount_axis: prep_distance}, speed=10, relative_move=True)

            # Drive Q down 3mm at fast speed - look into the pick up tip fuinction to find slow and fast: 10.0
            await self._gantry_mover.move_axes(axis_map={mount_axis: press_distance}, speed=10.0, relative_move=True)
            # 2.8mm at slow speed - cam action pickup speed: 5.5
            await self._gantry_mover.move_axes(axis_map={mount_axis: press_distance}, speed=5.5, relative_move=True)
            # retract cam : 11.05
            await self._gantry_mover.move_axes(axis_map={mount_axis: retract_distance}, speed=5.5, relative_move=True)
        else:
            await self._gantry_mover.move_axes(axis_map={mount_axis: -6}, speed=10, relative_move=True)

            # Drive Q down 3mm at fast speed - look into the pick up tip fuinction to find slow and fast: 10.0
            await self._gantry_mover.move_axes(axis_map={MotorAxis.AXIS_96_CHANNEL_CAM: prep_distance}, speed=10.0, relative_move=True)
            # 2.8mm at slow speed - cam action pickup speed: 5.5
            await self._gantry_mover.move_axes(axis_map={MotorAxis.AXIS_96_CHANNEL_CAM: press_distance}, speed=5.5, relative_move=True)
            # retract cam : 11.05
            await self._gantry_mover.move_axes(axis_map={MotorAxis.AXIS_96_CHANNEL_CAM: retract_distance}, speed=5.5, relative_move=True)

            # Lower tip presence
            await self._gantry_mover.move_axes(axis_map={mount_axis: 2}, speed=10)
            await self._gantry_mover.move_axes(axis_map={MotorAxis.AXIS_96_CHANNEL_CAM: ejector_push_mm}, speed=5.5, relative_move=True)
            await self._gantry_mover.move_axes(axis_map={MotorAxis.AXIS_96_CHANNEL_CAM: -1*ejector_push_mm}, speed=5.5, relative_move=True)
        # cache_tip
    ...
    async def execute(
        ...
        if not labware_validation.is_evotips(labware_definition.parameters.loadName):
            raise UnsupportedLabwareForActionError(f"Cannot use command: `EvotipSealPipette` with labware: {labware_definition.parameters.loadName}")
        ...
        tip_geometry = self._state_view.geometry.get_nominal_tip_geometry(pipette_id, labware_id, well_name)
        ...
        if params.tipPickUpParams and channels != 96:
            await self.relative_pickup_tip(pipette_id, params.tipPickUpParams, mount, press_fit=True)
        elif channels == 96:
            pick_up_params = params.tipPickUpParam if params.tipPickUpParams else TipPickUpParams()
            await self.relative_pickup_tip(pipette_id, pick_up_params, mount, press_fit=False)
        else:

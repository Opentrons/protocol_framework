...
from opentrons.protocol_api import ProtocolContext, Well, ParameterContext
from opentrons_shared_data.load import get_shared_data_root


metadata = {"protocolName": "LLD 1uL PCR-to-MVS-WITH-BASELINE-PARAM"}
...
def run(ctx: ProtocolContext) -> None:
    ...
    total_vol_per_well = (DILUENT_UL * TRIALS ) / len(diluent_wells)

    diluent_reservoir.load_liquid(diluent_wells, total_vol_per_well + DEAD_VOL_DILUENT, diluent)
    
    # load dye
    dye = ctx.define_liquid("dye", "#FF0000")
    src_labware.load_empty([w for w in src_labware.wells() if w.well_name != SRC_WELL])
    src_labware.load_liquid([src_labware[SRC_WELL]], (TARGET_UL * TRIALS) + DEAD_VOL_DYE, dye)
    ...
    for i in range(int(TRIALS/8)):
        ...
        diluent_pipette.aspirate(DILUENT_UL, diluent_well.bottom(.5))

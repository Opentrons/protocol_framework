...
#     )
#     protocol_core.comment(
#         f"Dropping tip from {instrument.mount} in {well_name} of {tiprack.name}"
#     )
...
#     else:
#         delta = Point(x=-10, y=0, z=0)
#     hardware._backend.set_active_current(
#         {Axis.of_main_tool_actuator(mount): config.current}
#     )
#     hardware.move_axes(
#         position={
#             Axis.of_main_tool_actuator(mount): hw_instrument.plunger_positions.drop_tip
#         },
#         speed=config.speed,
#     )
...
def run(ctx: ProtocolContext) -> None:
    ...
    if left_mount_pipette != "none":
        pipette = ctx.load_instrument(left_mount_pipette, "left", tip_racks=[tip_rack])
